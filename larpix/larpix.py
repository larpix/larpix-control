...
class Key(object):
    ...
    def keystring(self, val):
        if not self.is_valid_keystring(val):
    ...
    def from_dict(d):
        ...
        new_key = '-'.join([str(d['io_group']), str(d['io_channel']), str(d['chip_id'])])
        if not Key.is_valid_keystring(new_key):
            raise ValueError('keystring generated by specified fields is invalid: {}'.format(new_key))
        return Key(new_key)
