...
ctl.io = ZMQ_IO('io/testing_io.json') # specifies ip addresses + etc for io groups
if not ctl.io.ping():
    raise RuntimeError
...
for io_group in ctl.network:
    for io_channel in ctl.network[io_group]:
        us_subnetwork = ctl.network[io_group][io_channel]
        chip_keys = [key for key in us_subnetwork.nodes() if us_subnetwork.nodes[key]['root']]
        while chip_keys:
